...
const TableComponent = () => {
  ...
  const [removedListData, setRemovedListData] = useState("");
  ...
  useEffect(() => {
    if (removedListData) {
      fetch(`${BASE_URL}/lists/${removedListData}`, {
        mode: "cors",
        method: "DELETE",
        headers: {
          Accept: "application/json",
        ...
        .then((response) => {
          // trigger reload of lists
          setListData((oldListData) => oldListData.filter((list) => list.id !== removedListData));
          setRemovedListData("");
        })
        .catch((err) => {
          console.log("Error in fetch", err);
        ...
        return (
          <div key={index}>
            <table data-testid={todoList.id} data-testlist-name={todoList.name}>
              <thead key={index} className="TodoList">
                <tr>
                  <td colSpan={2}>{todoList.name}</td>
                  <td>
                    <button
                      id={todoList.id}
                      data-testid={`${todoList.id}-delete-list`}
                      onClick={(deleteList) => {
                        Object.keys(listData).forEach(function (key) {
                          if (listData[key]["id"] === deleteList.currentTarget.getAttribute("id")) {
                            setRemovedListData(listData[key]["id"]);
                          }
                        });
                      }}
                    >
              ...
              <tbody>
                {todoList.items &&
                  todoList.items.map(
                    (
                      ...
                      elementIndex: React.Key | null | undefined
                    ) => {
                      if (item) {
                        return (
                          <tr key={elementIndex}>
                            <td>
                              <input
                                ...
                                data-name={item.id}
                                id={todoList.id}
                                data-testid={`${todoList.name}-item-name`} //item name input
                                onChange={(e) => {
                                  Object.keys(listData).forEach(function (key) {
                                    // find corresponding list
                                    if (listData[key]["id"] === e.target?.getAttribute("id")) {
                                      Object.keys(listData[key]["items"]).forEach(function (
                                        secondaryKey
                                      ) {
                                        // find corresponding item
                                        if (
                                          listData[key]["items"][secondaryKey] &&
                                          listData[key]["items"][secondaryKey]["id"] ===
                                            e.target?.getAttribute("data-name")
                                        ) {
                                          listData[key]["items"][secondaryKey]["name"] =
                            ...
                            <td>
                              <input
                                ...
                                data-name={item.id}
                                id={`${todoList.id}`} //item completion checkbox
                                onChange={(item) => {
                                  Object.keys(listData).forEach(function (key) {
                                    if (listData[key]["id"] === item.target?.getAttribute("id")) {
                                      Object.keys(listData[key]["items"]).forEach(function (
                                        ...
                                        if (
                                          ...
                                          listData[key]["items"][secondaryKey]["isCompleted"] =
                                            item.target.checked;
                            ...
                            <td>
                              <button
                                data-name={item.id}
                                id={todoList.id} //item delete button
                                onClick={(removeItem) => {
                                  Object.keys(listData).forEach(function (key) {
                                    if (
                                      listData[key]["id"] ===
                                      removeItem.currentTarget?.getAttribute("id")
                                    ) {
                                      Object.keys(listData[key]["items"]).forEach(function (
                                        secondaryKey
                                      ) {
                                        ...
                                        if (
                                          listData[key]["items"][secondaryKey] &&
                                          listData[key]["items"][secondaryKey]["id"] ===
                                            removeItem.currentTarget?.getAttribute("data-name")
                                        ) {
                ...
                <tr>
                  <td colSpan={3}>
                    <button
                      id={todoList.id}
                      data-testid={`${todoList.name}-add-new-item`}
                      onClick={(newItem) => {
                        Object.keys(listData).forEach(function (key) {
